...
/**
 * If `raw` is a bare provider ID (no "/") that appears in the catalog, returns
 * the provider's default model qualified as `provider/model`. Otherwise null,
 * meaning the caller should use `raw` as-is (it may be a model name or alias).
 */
export function qualifyCatalogProviderRef(
  raw: string,
  ...
  // Collect each provider's first model entry
  const providerDefault = new Map<string, string>();
  for (const entry of catalog) {
    if (!providerDefault.has(entry.provider)) {
      providerDefault.set(entry.provider, entry.id);
    }
  }
  for (const [provider, model] of providerDefault) {
    if (provider.toLowerCase() === normalized) {
