...
当前支持的模型均可调用**本地工具**（web_search、web_fetch、exec、read、write、message），从而执行搜索、读写文件、运行命令等。

| 提供商类型                                                                         | 工具 | 实现方式                                                                                                                                                                  |
| ---------------------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Web — 原生 API**（DeepSeek、Claude、GLM）                                        | ✅   | 上游原生 `tools` / `tool_calls`                                                                                                                                           |
| **Web — 提示词注入**（Kimi、ChatGPT、Gemini、Grok、千问、千问CN、豆包、小米 MiMo） | ✅   | 通过提示词工程注入工具定义（[论文](https://arxiv.org/html/2407.04997v1)、[ComfyUI LLM Party](https://github.com/heshengtao/comfyui_LLM_party)）；正则解析响应中的工具调用 |
| **Web — 搜索引擎**（Perplexity）                                                   | ❌   | 仅搜索，不支持工具调用                                                                                                                                                    |
| **OpenRouter / OpenAI 兼容 API**                                                   | ✅   | 使用原生 `tools` / `tool_calls` 接口                                                                                                                                      |
| **Ollama**                                                                         | ✅   | 使用原生 `/api/chat` 的 tools                                                                                                                                             |

**Web 模型工具调用**通过统一中间件（`src/zero-token/tool-calling/`）实现：

1. 在用户消息中注入 6 个核心工具定义（~780 字符）
2. 正则解析模型响应中的 `tool_json` 块
3. 执行工具并将结果反馈给模型生成最终回复
